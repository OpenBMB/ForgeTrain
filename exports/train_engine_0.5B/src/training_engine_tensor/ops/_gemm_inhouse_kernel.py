"""In-house CuTeDSL GEMM kernel for H100 (SM90)

FP16/BF16 inputs, FP32 accumulation.
Compatible with CuTeDSL 4.4.2+ (make_tiled_tma_atom, tma_partition, cute.gemm).
"""
...
# ═══════════════════════════════════════════════════════════════════════
# Configuration — Per-shape optimization
# ═══════════════════════════════════════════════════════════════════════
TILE_SHAPE_MN = (128, 256)
CLUSTER_SHAPE_MN = (2, 1)

SHAPE_CONFIGS: dict = {
    "aop_fwd_dgrad": {"tile_mn": (128, 256), "cluster_mn": (1, 1)},
    "out_fwd":       {"tile_mn": (128, 128), "cluster_mn": (1, 2)},
    "out_dgrad":     {"tile_mn": (128, 256), "cluster_mn": (1, 1)},
    "qkv_fwd":       {"tile_mn": (128, 192), "cluster_mn": (1, 1)},
    "qkv_dgrad":     {"tile_mn": (128, 256), "cluster_mn": (1, 1)},
    "fc2_dgrad":     {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
    "fc1_fwd":       {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
    "fc2_fwd":       {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
    "aop_wgrad":     {"tile_mn": (128, 128), "cluster_mn": (2, 1)},
    "out_wgrad":     {"tile_mn": (128, 128), "cluster_mn": (2, 1)},
    "qkv_wgrad":     {"tile_mn": (128, 128), "cluster_mn": (1, 2)},
    "fc1_dgrad":     {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
    "fc1_wgrad":     {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
    "fc2_wgrad":     {"tile_mn": (128, 256), "cluster_mn": (2, 1)},
}


class M1PipelineGemmKernel:
    """Hopper GEMM kernel with TMA pipeline, WGMMA, CTA cluster, L2 swizzle."""
    ...
    def __init__(
        ...
        self.k_pipe_mmas = 2 if self.mma_warp_groups == 1 else 1
        ...
        self.is_a_mcast = cluster_shape_mn[1] > 1
        self.is_b_mcast = cluster_shape_mn[0] > 1
        ...
        self.b_layout = None
        self.c_layout = None
        self.tile_shape_mnk = None
        self.ab_stage = None
        self.prefetch_stage_limit = None
        self.epi_stage = None
    ...
    def _setup_attributes(self, a, b, c):
        """Setup from input tensors."""
        ...
        self.tile_shape_mnk = (self.tile_M, self.tile_N, tile_K)

        a_tile_bytes = self.tile_M * tile_K * self.a_dtype.width // 8
        b_tile_bytes = self.tile_N * tile_K * self.b_dtype.width // 8
        bytes_per_stage = a_tile_bytes + b_tile_bytes

        avail = self.smem_capacity // self.occupancy - self.buffer_align_bytes
        self.ab_stage = min(avail // bytes_per_stage, 7)
        self.ab_stage = max(self.ab_stage, 2)
        self.prefetch_stage_limit = self.ab_stage
        ...
        is_cooperative = self.mma_warp_groups > 1
    ...
    def __call__(
        ...
        tiled_mma = self._setup_attributes(a, b, c)

        # TMA atoms — G2S load for A, B; S2G store for C
        a_smem_layout_one = cute.slice_(self.a_smem_layout_staged, (None, None, 0))
        b_smem_layout_one = cute.slice_(self.b_smem_layout_staged, (None, None, 0))

        a_op = (
            cute.nvgpu.cpasync.CopyBulkTensorTileG2SOp()
            if self.num_mcast_ctas_a == 1
            else cute.nvgpu.cpasync.CopyBulkTensorTileG2SMulticastOp()
        )
        tma_atom_a, tma_tensor_a = cute.nvgpu.cpasync.make_tiled_tma_atom(
            a_op, a, a_smem_layout_one,
        ...
        b_op = (
            cute.nvgpu.cpasync.CopyBulkTensorTileG2SOp()
            if self.num_mcast_ctas_b == 1
            else cute.nvgpu.cpasync.CopyBulkTensorTileG2SMulticastOp()
        )
        tma_atom_b, tma_tensor_b = cute.nvgpu.cpasync.make_tiled_tma_atom(
            b_op, b, b_smem_layout_one,
            (self.tile_shape_mnk[1], self.tile_shape_mnk[2]),
            num_multicast=self.num_mcast_ctas_b,
        )

        epi_smem_layout_one = cute.slice_(self.epi_smem_layout_staged, (None, None, 0))
        tma_atom_c, tma_tensor_c = cute.nvgpu.cpasync.make_tiled_tma_atom(
            cute.nvgpu.cpasync.CopyBulkTensorTileS2GOp(),
            c, epi_smem_layout_one,
            self.epi_tile,
        )
        ...
        N = cute.size(b, mode=[0])
        L = max(cute.size(a, mode=[2]), 1)
        grid_M = cute.ceil_div(M, self.tile_M * self.cluster_shape_mn[0])
        grid_N = cute.ceil_div(N, self.tile_N * self.cluster_shape_mn[1])
        grid = [grid_M * self.cluster_shape_mn[0],
                grid_N * self.cluster_shape_mn[1], L]
    ...
    def kernel(
        ...
        # ── CTA Swizzle for L2 cache optimization ──
        ...
        prefetch_k_tile_cnt = cutlass.max(
            cutlass.min(self.prefetch_stage_limit, k_tile_cnt), 0
        )
        ...
        for k_tile in cutlass.range(k_pipe_mmas, k_tile_cnt, 1, unroll=1):
            ...
            # Interleave TMA loads with compute
...
HopperWgmmaGemmKernel = M1PipelineGemmKernel


# ═══════════════════════════════════════════════════════════════════════
# Persistent kernel — per-shape optimization configs
# ═══════════════════════════════════════════════════════════════════════
PERSISTENT_CONFIGS: dict = {
    # ── fwd / dgrad — BF16 output ──
    "fc1_fwd":   {"tile_mn": (128, 256), "cluster_mn": (2, 1), "swizzle": 8, "raster_m": False},   # 730 TF (+2.3% vs cuBLAS)
    "fc1_dgrad": {"tile_mn": (128, 256), "cluster_mn": (1, 2), "swizzle": 2, "raster_m": True},    # 797 TF (-0.4%)
    "fc2_fwd":   {"tile_mn": (128, 256), "cluster_mn": (2, 1), "swizzle": 4, "raster_m": True},    # 775 TF (+5.5%) — tuned sw=1→4, raster→True
    "fc2_dgrad": {"tile_mn": (128, 256), "cluster_mn": (2, 1), "swizzle": 4, "raster_m": False},   # 761 TF (+0.6%)
    "aop_fwd":   {"tile_mn": (128, 256), "cluster_mn": (1, 2), "swizzle": 8, "raster_m": False},   # 679 TF (+3.8%)
    "aop_dgrad": {"tile_mn": (128, 256), "cluster_mn": (1, 1), "swizzle": 2, "raster_m": False},   # 675 TF (+1.4%)
    # ── wgrad — FP32 output, L=1 (no split-K) ──
    "fc1_wgrad": {"tile_mn": (128, 256), "cluster_mn": (1, 2), "swizzle": 1, "raster_m": False},   # 811 TF (-1.2%) — tuned sw=8→1
    "fc2_wgrad": {"tile_mn": (128, 256), "cluster_mn": (2, 1), "swizzle": 2, "raster_m": True},    # 806 TF (-0.4%)
    "aop_wgrad": {"tile_mn": (64, 128),  "cluster_mn": (2, 1), "swizzle": 2, "raster_m": True},    # single-pass 513 TF
    # split-K variants for aop_wgrad (M=N=1024, K is split)
    "aop_wgrad_sk2": {"tile_mn": (128, 128), "cluster_mn": (1, 2), "swizzle": 2, "raster_m": False},  # split-K=2, K_eff=20480 — tuned sw=1→2
    "aop_wgrad_sk4": {"tile_mn": (64, 128),  "cluster_mn": (2, 1), "swizzle": 2, "raster_m": True},   # split-K=4, K_eff=10240
}


class M1PersistentGemmKernel:
    """Hopper persistent GEMM with warp specialization, TMA, WGMMA.

    Compared to M1PipelineGemmKernel (non-persistent):
    - CTA blocks stay resident on SM and process multiple output tiles via
      StaticPersistentTileScheduler (reduces launch overhead, improves SM occupancy).
    - Dedicated DMA warp group for TMA loads, separate from MMA warp groups
      (better overlap of memory access and compute).
    - Independent sC SMEM allocation (not recast from sA) with per-shape
      swizzle/raster tuning via PersistentTileSchedulerParams.
    """
    ...
    def __init__(
        ...
        self.k_pipe_mmas_cfg = k_pipe_mmas
    ...
    @staticmethod
    def _make_smem_layouts(tile_shape_mnk, epi_tile,
    ...
    def kernel(
        ...
        # ── Warp specialization: DMA vs MMA ──
        ...
        # ── DMA warp group: TMA producer loop ──
        ...
        # ── MMA warp groups: consumer loop + epilogue ──
        if not is_dma_warp_group:
            ...
            k_pipe_mmas = self.k_pipe_mmas_cfg
            ...
            while work_tile.is_valid_tile:
                ...
                # ── Epilogue: accumulator → sC → GMEM via TMA store ──
...
HopperPersistentGemmKernel = M1PersistentGemmKernel
